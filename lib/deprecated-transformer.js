...
function removeMatch(string, match) {
  ...
  return string.slice(0, match.index).trimEnd()
...
export default class DeprecatedTransformer
  ...
  transformSchema(schema) {
    ...
    if (typeof description !== 'string'
      ...
      || !/deprecat(?:e|ion)/i.test(description)) {
      return newSchema;
    }
    ...
    for (const deprecationRegexp of [
      /:(?<deprecated>[^:\s]\S*) to be deprecated, use :(?<replacement>\S+)/,
      /\(this property is deprecated - see (?<replacement>\S+)\)/,
      /\*?DEPRECATED\. Please use (?<replacement>\S+) instead/,
      /DEPRECATED - Use (?<replacement>\S+) instead\.\s*/,
      /Deprecated\. Use `(?<replacement>\S+)`/,
      /\(Deprecated\) Use `(?<replacement>\S+)`/,
      /DEPRECATED: Use :(?<replacement>\S+)\.\s*/,
      /\u2014 DEPRECATED, please use "(?<replacement>\S+)" instead/,
      /Note that use of this parameter is deprecated\. Please use `(?<replacement>\S+)` instead\./,
      /Note: this field is now deprecated and will mirror the value of (?<replacement>\S+) until it is no longer supported\./,
      /This field is DEPRECATED as of \w+ [0-9]{1,2}, [0-9]{4} and will no longer be supported as of \w+ [0-9]{1,2}, [0-9]{4}\./,
